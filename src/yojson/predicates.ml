let has_error_code assoc =
  match List.assoc_opt "code" assoc with Some (`Int _) -> true | _ -> false
...
let has_params assoc =
  match List.assoc_opt "params" assoc with Some _ -> true | None -> false

let has_id assoc =
  match List.assoc_opt "id" assoc with Some (`String _) -> true | _ -> false
...
let has_result assoc =
  match List.assoc_opt "result" assoc with Some _ -> true | None -> false
